...
void print_robots_positions(tw_lp *lp)
{
  ...
  printf("Number of remained  boxes: %d\n", g_remained_boxes);
...
void robot_init(robot_state *s, tw_lp *lp)
{
  ...
  {
    if (desired_row == 1 && desired_col != 9)
    {
      desired_col++;
      continue;
    }
    if (desired_col == 9 && desired_row != 8)
    {
      desired_row++;
      continue;
    }
...
void generator_event_handler(generator_state *s, tw_bf *bf, message *in_msg, tw_lp *lp)
{
  ...
  if (s->current_cargo + 1 == MAX_CARGO_NUMBER)
    ts = g_robot_calc_time;
  // time has not come
  else if (lp->pe->cur_event->recv_ts < s->cargo_timetable[s->current_cargo][0])
    ts = g_robot_calc_time;
  // all is OK
  else
    ret_cargo = s->cargo_timetable[s->current_cargo++][1];
  
  // Calculation of remaining boxes
  int remains = 0;
  while (lp->pe->cur_event->recv_ts > 
         s->cargo_timetable[s->current_cargo + remains][0])
    remains++;
  g_remained_boxes = remains;
